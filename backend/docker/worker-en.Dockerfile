...
RUN bash ./models/download-ggml-model.sh small.en \
 ...
 && "$QUANT" ./models/ggml-small.en.bin ./models/ggml-small.en-q8_0.bin q8_0 \
 && rm ./models/ggml-small.en.bin
...
COPY --from=builder /build/whisper.cpp/models/ggml-small.en-q8_0.bin /opt/whisper/model.bin
